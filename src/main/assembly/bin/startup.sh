...
JAVA_OPT="-server -Xms1024m -Xmx1024m -Xmn512m -XX:MetaspaceSize=64m -XX:MaxMetaspaceSize=128m"
...
# 打印启动命令
STARTUP_LOG="${STARTUP_LOG}application background startup command: nohup java ${JAVA_OPT} -jar ${BASE_PATH}/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} > ${LOG_PATH} 2>&1 &\n"
